...
#undef DNManagedObjectXMLSyncDebug

#pragma mark -
#pragma mark DNManagedObjectXMLSyncCrossReference

@interface DNManagedObjectXMLSyncCrossReference : NSObject
{
...
@implementation DNManagedObjectXMLSyncCrossReference
...
@interface DNManagedObjectXMLSync (Private)
...
@implementation DNManagedObjectXMLSync
...
- (void)saveCrossReference:(NSDictionary*)attributeDict fromManagedObject:(NSManagedObject*)object throughProperty:(NSRelationshipDescription*)property {	
	...
	DNManagedObjectXMLSyncCrossReference *reference = [[[DNManagedObjectXMLSyncCrossReference alloc] initWithManagedObject:object
...
#ifdef DNManagedObjectXMLSyncDebug
...
- (BOOL)mergeModelObject:(NSManagedObject*)newObject withExistingObject:(NSManagedObject*)existingObject {
	...
	NSComparisonResult compareResult = (NSComparisonResult)[oldRevision performSelector:compare withObject:newRevision];
...
- (void)findCrossReferences {
	...
	for (DNManagedObjectXMLSyncCrossReference *crossReference in _pendingIdMapping) {
		if ([crossReference findReferencesInContext:self.managedObjectContext]) {
			[completedReferences addObject:crossReference];
		}
	}
	
	for (DNManagedObjectXMLSyncCrossReference *finishedReference in completedReferences) {
